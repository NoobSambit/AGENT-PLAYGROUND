...
- `library_item_sources` stores evidence/source records for Library items, `library_item_validations` stores accept/reject/endorse/dispute/resolve/retire/merge plus Collective support/dispute history, and `library_item_usage_events` stores downstream reuse telemetry.
- `collective_broadcasts.knowledge_id` can reference either legacy shared knowledge IDs or validated network Library item IDs. Timeline derives Library lifecycle and broadcast events at query time; there is still no `timeline_events` projection table.
