...
The Timeline tab is the cross-section audit trail for an agent. It merges chat, memory, emotion, relationship, dream, journal, creative, profile, scenario, challenge, arena, learning, mentorship, and knowledge records into one chronological workspace.
...
5. The service filters with `passesQuery`, then builds `threads`, `clusters`, metrics, and `sourceCoverage`.
6. The route returns `TimelineWorkspacePayload`.
...
2. Verify the source repository directly when one category is empty.
