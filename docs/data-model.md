...
- Timeline V2 is query-time derived from existing feature tables, including Knowledge Library lifecycle and validation rows. Do not add a `timeline_events` table unless profiling shows the adapter aggregation needs a rebuildable cache.
...
- The Knowledge Library rebuild stores reusable claims in `library_items`, evidence in `library_item_sources`, lifecycle decisions in `library_item_validations`, and reuse telemetry in `library_item_usage_events`. Only validated items with payload `contextPolicy.allowPromptUse=true` should be eligible for downstream prompt context. Merge and supersede governance keeps old items as retired audit history and links source refs to the replacement item instead of deleting provenance.
- Collective Intelligence reads validated prompt-eligible `scope='network'` Library items as the canonical network knowledge layer. Collective support/dispute actions append Library validation records for Library-backed entries, while legacy `shared_knowledge` rows remain compatibility data.
