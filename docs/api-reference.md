...
- `GET /api/agents/[id]/library/items/[itemId]` returns the full Library item detail with sources, validations, usage events, related governance links, and duplicate suggestions.
- `POST /api/agents/[id]/library/items/[itemId]/actions` supports `accept`, `reject`, `endorse`, `dispute`, `resolve`, `retire`, `merge`, and `supersede`. Rationale is required for `reject`, `dispute`, `resolve`, `retire`, `merge`, and `supersede`; `merge`/`supersede` require `targetItemId`; invalid transitions return `409`.
- `GET /api/collective-intelligence` now includes validated, prompt-eligible network Library items alongside legacy `shared_knowledge` rows. Library-backed entries include `knowledgeSource='library_item'`, `libraryItemId`, lifecycle metadata, and a Library detail link.
- `POST /api/collective-intelligence` action `broadcast` can broadcast a validated network Library item by passing `knowledgeId` and `knowledgeSource='library_item'`. Action `validate` writes Library-backed support/dispute decisions to `library_item_validations`; legacy shared rows still use `/api/knowledge` compatibility mutation.
- `GET /api/agents/[id]/timeline` includes Knowledge Library item, validation, and Library-backed broadcast events in the `knowledge` source family with source refs and related Library item refs.
