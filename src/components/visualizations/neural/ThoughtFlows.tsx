  ...
  return (
    <>
      {streams.map((stream, i) => (
        <ThoughtStream key={`input-${i}`} flow={stream} particleCount={8} />
      ))}
    </>
  )
}
...
function OutputStreams({ count = 8, isActive }: { count?: number; isActive: boolean }) {
  ...
  return (
    <>
      {streams.map((stream, i) => (
        <ThoughtStream key={`output-${i}`} flow={stream} particleCount={10} />
      ))}
    </>
  )
}
...
function ProcessingStreams({
  ...
  return (
    <>
      {streams.map((stream, i) => (
        <ThoughtStream key={`processing-${i}`} flow={stream} particleCount={6} />
      ))}
    </>
  )
}
...
export function ThoughtFlowVisualization({
  ...
  return (
    <group>
      ...
      {flows.map((flow, i) => (
        <ThoughtStream key={`custom-${i}`} flow={flow} />
      ))}
    </group>
  )
}
