...
export class MemoryChain {
  ...
  private calculateToolImportance(toolName: string, input: string, output: string): number {
    let importance = 6 // Tools are generally more important

    // Increase importance for summarization or complex operations
    if (toolName.includes('summarize') || toolName.includes('extract')) {
      importance += 2
    }

    // Increase importance for persona adjustments
    if (toolName.includes('persona') || toolName.includes('personality')) {
      importance += 1
    }

    // Longer inputs/outputs imply higher significance
    if (input.length > 200 || output.length > 300) {
      importance += 1
    }
