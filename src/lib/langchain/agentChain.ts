...
export class AgentChain {
  ...
  async generateResponse(
    ...
    try {
      ...
      const shouldUseTools = this.config.enableTools &&
        this.shouldUseTools(userInput, agentData.persona)
  ...
  private createSystemPrompt(_agent: any, memoryMessages: BaseMessage[]): string {
    ...
    return baseChain.createSystemPrompt(
      _agent.persona,
      _agent.goals || [],
      memoryContext,
      personalityContext
  ...
  private shouldUseTools(input: string, _persona: string): boolean {
  ...
  private async useTools(
    ...
    try {
      // Try summarization if input is long
      if (input.length > 150) {
        const summary = await this.toolExecutor.summarize(input)
        if (summary) {
      ...
      const keywords = await this.toolExecutor.extractKeywords(input)
