...
function emotionEvent(agent: AgentRecord, event: EmotionalEvent): TimelineEventV2 {
  ...
  return makeEvent({
    ...
    content: event.context || event.explanation,
...
function libraryEvents(agent: AgentRecord, details: LibraryItemDetail[], broadcasts: Awaited<ReturnType<typeof BroadcastRepository.listRecent>>): TimelineEventV2[] {
  const itemEvents = details.map((detail) => makeEvent({
    ...
    content: detail.item.body || detail.item.claim,
    ...
    return [
      ...
      },
      {
        ...
        load: async (agent) => (await DreamWorkspaceRepository.listDreamsForAgent(agent.id, { savedOnly: true, limit: 40 })).map((dream) => makeEvent({
          ...
          content: [dream.summary, dream.interpretation?.summary, dream.scenes?.[0]?.body].filter(Boolean).join('\n\n'),
