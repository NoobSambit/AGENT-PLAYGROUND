...
function memoryToFirestoreDoc(memory: CreateMemoryData | UpdateMemoryData): Omit<MemoryDocument, 'timestamp'> & { timestamp?: Timestamp } {
  return {
    agentId: (memory as CreateMemoryData).agentId || '',
    type: (memory as CreateMemoryData).type || 'conversation',
    content: 'content' in memory ? memory.content || '' : '',
    summary: 'summary' in memory ? memory.summary || '' : '',
    keywords: 'keywords' in memory ? memory.keywords || [] : [],
    importance: 'importance' in memory ? memory.importance || 5 : 5,
    context: 'context' in memory ? memory.context || '' : '',
...
export class MemoryService {
  ...
  static async updateMemory(id: string, updates: UpdateMemoryData): Promise<boolean> {
    try {
      const docRef = doc(db, MEMORIES_COLLECTION, id)
      await updateDoc(docRef, memoryToFirestoreDoc(updates))
      return true
    } catch (error) {
