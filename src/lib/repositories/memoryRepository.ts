...
function mapMemoryRow(row: MemoryRow): MemoryRecord {
  return {
    ...
    canonicalValue: row.canonicalValue || undefined,
    confidence: row.confidence ?? undefined,
    evidenceRefs: row.evidenceRefs || undefined,
    supersedes: row.supersedes || undefined,
    lastConfirmedAt: row.lastConfirmedAt ? asIsoString(row.lastConfirmedAt) : undefined,
...
export class MemoryRepository {
  ...
  static async create(record: MemoryRecord): Promise<MemoryRecord> {
    const [row] = await getDb().insert(memories).values({
      ...
      lastConfirmedAt: record.lastConfirmedAt ? asIsoString(record.lastConfirmedAt) : null,
  ...
  static async update(id: string, updates: Partial<MemoryRecord>): Promise<boolean> {
    const [row] = await getDb().update(memories).set(compact({
      ...
      canonicalKey: updates.canonicalKey,
      canonicalValue: updates.canonicalValue,
      confidence: updates.confidence,
      evidenceRefs: updates.evidenceRefs,
      supersedes: updates.supersedes,
      lastConfirmedAt: updates.lastConfirmedAt ? asIsoString(updates.lastConfirmedAt) : undefined,
