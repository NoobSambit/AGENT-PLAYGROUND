...
export const libraryItems = pgTable('library_items', {
  ...
  createdFromFeature: text('created_from_feature').$type<LibrarySourceType>().notNull(),
  primarySourceType: text('primary_source_type').$type<LibrarySourceType>().notNull(),
  primarySourceId: text('primary_source_id').notNull(),
  tags: text('tags').array().notNull().default(sql`'{}'::text[]`),
  relatedAgentIds: text('related_agent_ids').array().notNull().default(sql`'{}'::text[]`),
  ...
  createdAt,
  updatedAt: timestamp('updated_at', { withTimezone: true, mode: 'string' }).notNull(),
  payload: jsonb('payload').$type<LibraryItemPayload>().notNull().default(sql`'{}'::jsonb`),
}, (table) => [
  index('library_items_agent_status_updated_idx').on(table.agentId, table.status, table.updatedAt),
  index('library_items_scope_status_confidence_idx').on(table.scope, table.status, table.confidence),
  index('library_items_category_status_confidence_idx').on(table.category, table.status, table.confidence),
  index('library_items_primary_source_idx').on(table.primarySourceType, table.primarySourceId),
  index('library_items_created_from_feature_idx').on(table.createdFromFeature, table.createdAt),
  index('library_items_tags_gin_idx').using('gin', table.tags),
])

export const libraryItemSources = pgTable('library_item_sources', {
  id: text('id').primaryKey(),
  itemId: text('item_id').notNull().references(() => libraryItems.id, { onDelete: 'cascade' }),
  sourceType: text('source_type').$type<LibrarySourceType>().notNull(),
  sourceId: text('source_id').notNull(),
  sourceTitle: text('source_title'),
  sourceUrl: text('source_url'),
  sourceTimestamp: timestamp('source_timestamp', { withTimezone: true, mode: 'string' }),
  evidenceSummary: text('evidence_summary').notNull(),
  quote: text('quote'),
  createdAt,
  payload: jsonb('payload').$type<LibraryItemSource['payload']>().notNull().default(sql`'{}'::jsonb`),
}, (table) => [
  index('library_item_sources_item_idx').on(table.itemId),
  index('library_item_sources_source_idx').on(table.sourceType, table.sourceId),
])

export const libraryItemValidations = pgTable('library_item_validations', {
  id: text('id').primaryKey(),
  itemId: text('item_id').notNull().references(() => libraryItems.id, { onDelete: 'cascade' }),
  ...
  createdAt,
  payload: jsonb('payload').$type<LibraryValidation['payload']>().notNull().default(sql`'{}'::jsonb`),
}, (table) => [
  index('library_item_validations_item_created_idx').on(table.itemId, table.createdAt),
  index('library_item_validations_agent_created_idx').on(table.agentId, table.createdAt),
  index('library_item_validations_verdict_created_idx').on(table.verdict, table.createdAt),
])

export const libraryItemUsageEvents = pgTable('library_item_usage_events', {
  id: text('id').primaryKey(),
  itemId: text('item_id').notNull().references(() => libraryItems.id, { onDelete: 'cascade' }),
  agentId: text('agent_id').references(() => agents.id, { onDelete: 'set null' }),
  consumerFeature: text('consumer_feature').$type<LibrarySourceType>().notNull(),
  consumerSourceId: text('consumer_source_id'),
  query: text('query'),
  relevanceScore: doublePrecision('relevance_score').notNull().default(0),
  usedAt: timestamp('used_at', { withTimezone: true, mode: 'string' }).notNull(),
  payload: jsonb('payload').$type<LibraryUsageEvent['payload']>().notNull().default(sql`'{}'::jsonb`),
}, (table) => [
  index('library_item_usage_events_item_used_idx').on(table.itemId, table.usedAt),
  index('library_item_usage_events_agent_feature_used_idx').on(table.agentId, table.consumerFeature, table.usedAt),
])
