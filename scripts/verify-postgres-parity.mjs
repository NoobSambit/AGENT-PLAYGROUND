...
function deriveAgentCounters(data) {
  ...
  for (const agentDoc of data.collections?.agents || []) {
    counters.set(agentDoc.id, {
      memories: 0,
      relationships: 0,
      creativeWorks: (agentDoc.subcollections?.creative_works || []).length,
...
async function main() {
  ...
  await withPgClient(async (client) => {
    const tableCounts = {
      ...
      creative_works: await countTable(client, 'creative_works'),
    ...
    const expectedTableCounts = {
      ...
      creative_works: expectedSummary.subcollections.creative_works,
